...
describe('Event Service', () => {
    ...
    describe('Requires existing events', () => {
        ...
        beforeEach(async () => {
            ...
            const eventInsertResult = await queryBuilder.insert()
                .into(EventModel)
                .values([
                    {title: 'test event 1', seriesId, description: 'this is only a test'},
                    {title: 'test event 2', description: 'this is only a test without a series'}
                ])
                .execute();
